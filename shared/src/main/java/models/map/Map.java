...
public class Map implements DailyUpdate, Serializable {
    ...
    public Coord getRandomCoord(Random random) {
        return Coord.getRandomCoord(getMaxX(), getMaxY(), random);
    }
    ...
    public boolean nextDay(Game g) {
        if (mapType == MapType.MINES) {
            ...
            mb.buildRandomTile(this, foragingTypes, 1, 3, g.rand);
        }

        if (g.getWeather() == Weather.STORM) {
            for (int i = 0; i < NUMBER_OF_THORS_PER_DAY; i++)
                thor(getRandomCoord(g.rand));
        }
